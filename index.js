...
app.get('/export', function (req, res) {
  var tweetsToExport = [
      {
        "id": 3,
        "code": '909812257336565761',
        "text": '@jonathanchait Reps. are desperate to give Trump a win to stop the lover affair with Schumer/Pelosi',
        "username": 'adamtsilverman',
        "name": 'adam silverman',
        "date": '2017-09-18',
        "hour": '12:12:00',
        "likes": 0,
      ...
      {
        "id": 2,
        "code": '909812257353236481',
        "text": 'RT @rolandscahill: Trump just crossed 56th street in Manhattan to a chorus of boos. I love NYC!',
        "username": 'MicaelaGarzoni',
        "name": 'Micaela Garzoni',
        "date": '2017-09-18',
        "hour": '12:12:00',
        "likes": 0,
        "rts": 0,
  ...
  for (var i in titles) {
    ...
    // content
    for(var j in tweetsToExport){
        var k = 0
        for(var obji of Object.keys(tweetsToExport[j])){
            ws.cell(parseInt(j) + 2, parseInt(k)+1).string(!tweetsToExport[j][obji] ? '' : tweetsToExport[j][obji].toString());
            k++;
        }
    }
